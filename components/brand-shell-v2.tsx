    ...
    <header className="site-header">
      <div className="header-inner">
        ...
        <nav className="desktop-nav" aria-label="Navegação principal">
          {lines.map(line => <Link key={line.id} href={line.path} aria-current={path === line.path ? 'page' : undefined} className={isActive(line.path) ? 'active' : ''}>{line.name}</Link>)}
        </nav>
        <div className="header-actions">
          ...
          <details key={path} ref={menuRef} className="vx-mobile-nav">
            ...
            <nav aria-label="Navegação móvel">
              ...
              <Link href="/ajuda" aria-current={path === '/ajuda' ? 'page' : undefined} onClick={closeMenu}>Ajuda<ArrowUpRight aria-hidden="true" size={15} /></Link>
            </nav>
          </details>
        </div>
      </div>
...
export function BrandFooter() {
  return <footer className="site-footer">
    <div className="footer-main container">
      <div className="footer-brand"><BrandLogo light /><p>Objetos com personalidade.<br />Do achado à peça só sua.</p></div>
      <div><h3>Explore</h3>{lines.map(line => <Link key={line.id} href={line.path}>{line.name}</Link>)}</div>
      <div><h3>Conheça</h3><Link href="/sobre">Sobre a Vektua XYZ</Link><Link href="/como-funciona">Como funciona</Link><Link href="/ajuda">Perguntas frequentes</Link><Link href="/contato">Atendimento</Link></div>
      <div><h3>Informações</h3><Link href="/politicas/entrega">Entrega</Link><Link href="/politicas/trocas-devolucoes">Trocas e devoluções</Link><Link href="/politicas/privacidade">Privacidade</Link><Link href="/politicas/termos">Termos de uso</Link></div>
    </div>
    <div className="footer-bottom container"><span>© 2026 Vektua XYZ</span><span>Identidade digital em implementação · sem liberação comercial</span><span>Do achado à peça só sua.</span></div>
  </footer>;
}
